...
hdr *defragment_allocs(hdr *cur_node, size_t size, int free_check) {
    ...
    /* return the merged, defragmented memory allocation*/
    return cur_node;
}
...
void *realloc_func(void *ptr, size_t size) {
    ...
    if(op_hdr->alloc_size >= size) {
        ...
        NOTE: most likely redundant, realloc() has special case for this*/
        if(!(op_hdr->alloc_size = size))
    ...
    /* again, if possible, attempt in-place size adjustment*/
    if(op_hdr->alloc_size >= size) {
    ...
    after, copy the data from ptr to the new block and
    mark the old block as free*/
    // snprintf(out_str, 100, "realloc forced to move buffer\n");
    // write(STDOUT_FILENO, out_str, strlen(out_str));
    if(!(new_ptr = malloc(size))) {
        return NULL;
    }
    // snprintf(out_str, 100, "malloc successful\n");
    // write(STDOUT_FILENO, out_str, strlen(out_str));
    memcpy(new_ptr, ptr, size);
    /* since defragmented memory wasn't used, might
    as well free it */
    op_hdr->free_flag = FREE_MEM;
    return new_ptr;
